      ...
      <div className="page">
        ...
        <form className="toolbar" onSubmit={handleSearchSubmit}>
          ...
          <button type="button" className="sort-btn" onClick={toggleSort}>
            Sort Price: {sort === "asc" ? "High to Low" : "Low to High"}
          </button>
        </form>

        {loading ? (
          <p className="empty-state">Loading cabs...</p>
        ) : cabs.length === 0 ? (
          <p className="empty-state">No cabs found. Try a different search.</p>
        ) : (
          <div className="cab-grid">
            {cabs.map((cab) => (
              <div className="cab-card" key={cab._id}>
                <img
                  src={cab.carImage ? `http://localhost:8000${cab.carImage}` : "https://placehold.co/300x150/f5a623/1a1300?text=Cab"}
                  alt={cab.carModel}
                />
                <div className="cab-body">
                  <strong>{cab.carModel}</strong>
                  <span className="cab-meta">Type: {cab.carType}</span>
                  <span className="cab-meta">Car No: {cab.carNo}</span>
                  <span className="cab-meta">Driver: {cab.driverName}</span>
                  <span className="cab-meta">Fare: ₹{cab.price}/Km</span>
                </div>
                <div className="cab-actions">
                  <button
                    className="btn-book"
                    onClick={() => navigate(`/bookcab/${cab._id}`)}
                  >
                    Book Cab
                  </button>
                </div>
